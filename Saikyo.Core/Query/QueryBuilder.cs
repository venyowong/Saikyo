...
namespace Saikyo.Core.Query
{
    public class QueryBuilder<T> where T : new()
    {
        ...
        internal QueryBuilder(Collection<T> collection, string key, Symbol ope, string value)
        {
        ...
        public QueryBuilder(Collection<T> collection, string condition)
        {
            this.collection = collection;
            if (!string.IsNullOrWhiteSpace(condition))
            {
                    ...
                    switch (expType)
                    {
                        ...
                        case ExpType.Parenthesis:
                            switch (condition[i])
                            {
                                case ')':
                                    expType = ExpType.Exp;
                                    this.queryBuilders.Add(new QueryBuilder<T>(this.collection, exp));
                                    exp = string.Empty;
                                    continue;
                                default:
                                    exp = exp + condition[i];
                        ...
                        case ExpType.Exp:
                            switch (condition[i])
                            {
                                ...
                                case '|':
                                    ...
                                    {
                                        this.queryBuilders.Add(new QueryBuilder<T>(this.collection, key, ope, value));
                                        key = string.Empty;
                                        ope = Symbol.Invalid;
                                        value = string.Empty;
                                    }
                ...
                switch (expType)
                {
                    ...
                    case ExpType.SingleQuotes:
                    case ExpType.Value:
                        this.queryBuilders.Add(new QueryBuilder<T>(this.collection, key, ope, exp));
                        break;
                    case ExpType.Key:
                    case ExpType.Operator:
        ...
        public QueryResult<T> Build()
        {
            if (this.type == QueryType.Base)
            {
                ...
                if (gather == null)
                {
                    return new QueryResult<T>(this.collection);
                }
                ...
                return new QueryResult<T>(this.collection, this.key, values);
            }
            ...
            else
            {
                ...
                List<Column> keys = gather.GetAllBlocks();
                return new QueryResult<T>(this.collection, this.collection.Key, keys);
            }
        }
    }
}
